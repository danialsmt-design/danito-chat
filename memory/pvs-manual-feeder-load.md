...
**UNITS — the file's count is PER PANEL (2026-09-24).** A Sony pen-drive Mount Step (and JUKI QTY) is the placement count of the machine program, which mounts the WHOLE PANEL — never multiply it by boards-per-panel. The DB feeder map QtyPerUnit is per child board (× panel factor). Getting this wrong charged L1 4× and L3 6× from 22 Sep 2026 (every reel to zero early, false attrition, blind forecast). Fixed in `863f3ec` (`PerPanelMount`). See [[pvs-feeder-list-is-truth]].

**RULE 2026-09-24 (Danial): a pen-drive list applies only while the machine runs that program** — same model + side as the file's label; otherwise that machine uses the DB feeder map (`ProgramNames.ManualListApplies`, audit `ManualListIgnored`, load refused with PROGRAM-MISMATCH unless forced). Pen drive = the exception for a breakdown reshuffle, not a routine at every model change (L1/L3/L4 supervisors were doing that). Reload-from-DB = setup.html → '↻ Reload from DB' (supervisor badge) → `POST /api/feeders/manual/clear`. Built `HEAD` 2026-09-24.
