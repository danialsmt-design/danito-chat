...
metadata: 
  ...
  modified: 2026-08-18T22:09:45.035Z
---
...
**⚠️ BEHAVIOUR (Danial feedback 2026-08-19: "why do i have to tell you where to get it from"):** whenever anything on the MCS side is mentioned or changes — the production slip / QR, entry pages, schema, a "MCS did X" — **go READ the source of truth FIRST, before asking Danial or relying on screenshots.** The details are already here: the ledger **`Documents/Dantec/MCS/COORDINATION.md`** (MCS logs notes ADDRESSED to the PVS session there — e.g. the 2026-08-18 slip-QR change + 3 PVS action items) and the **MCS app itself at `Documents/Dantec/MCS/app/`** (`wwwroot/slip.html` = the real slip + QR string, `stock-out.html`, `returns.html`, `tray.html`, `Program.cs`). Reason from the shared channel; don't make Danial hand-feed what's already written down. **How to apply:** MCS mentioned → open COORDINATION.md + the relevant `MCS/app` file → THEN answer. See [[pvs-board-flow]] (slip/QR authoritative format lives in slip.html), [[pvs-system-brain]].

**PVS shows the MCS plan (2026-09-09, `fbf2122`, L1):** PVS is a read-only mirror of the MCS scheduler — `PlanFeedService` polls `<mcs>/api/schedule` (NAS :8090, same base URL as `robot.dispatcherUrl`) every 5 min and the verify page shows the day's lots for that line (Today/Tomorrow/+2). MCS stays the single writer of the plan; PVS never writes back.
**Late lines are MCS's job (Danial, 2026-09-09):** when a line runs non-Canon (parked) and its Canon lots slip, do NOT touch PVS or the park — MCS detects the line's real state and moves the start times. PVS only mirrors the plan.
