...
**Update 2026-09-23:** re-pointed again after the UCG cutover: L1 `.141` (Ethernet DHCP), L2 `.147`, L4 `.144`, L5 `.157` (static Wi-Fi), L3 `.83` (Wi-Fi now DHCP). L3's static Wi-Fi `.126` was *Duplicate* (Windows disabled it → 169.254) — check `Get-NetIPAddress … AddressState` first when a line has no LAN path; L3 also carried a stale static `.133` on its disconnected Ethernet that owned the 192.168.0.0/24 route. Restarting the container over SSH: `echo <pw> | sudo -S -p '' /usr/local/bin/docker compose up -d --force-recreate` (plain `sudo docker` fails: no tty + docker not on sudo's path); put the SSH command in a .ps1 file (inline quoting breaks). Durable fix still = UCG DHCP reservations per line-PC MAC.
