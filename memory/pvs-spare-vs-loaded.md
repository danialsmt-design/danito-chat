...
**RULE CHANGE 2026-09-07 (Danial):** "If the parts are not on the feeder list for each machine they should not be decremented; all feeders which are not on the current selected model should unload automatically, and should not be in the exhaust card." This REPLACES the earlier "every loaded feeder must decrement" grounding for OFF-LIST feeders: a reel still mapped to a feeder the selected model/side doesn't use is a leftover from an earlier run, not production. At baseline (model select / restart / refresh) PVS now auto-unloads them (mapping cleared to `feeder-reels.json.auto-unload-backup.json`, remainder kept by UID, StockOut untouched, audited `AutoUnload`) so they are never configured into live inventory. Found on L1: 24 of 56 exhaust rows / 16 parts were off-list, drained 4 pcs/panel for nothing, 3 phantom run-outs. Feeders ON the list still decrement whether or not the reel was scanned (rule unchanged there).

**REEL BALANCE = StockOut for the loaded reel UID, on EVERY line (Danial 2026-09-14):** the `syncStockOuts` pilot flag is gone (`6d554cb`, all lines). PVS takes the start qty from StockOut at scan, counts down, and writes the balance back to `StockOuts.Quantity` by UID every 5 min and right after any correction; a completed parts-change retires the outgoing reel (zero + ConsumedReels + attrition) everywhere. Health signal `stockOut` + a 🗄 chip on verify.html show the last successful write. Before this, L2–L5 never wrote back (created off, never revisited) — StockOut there was the issued qty all run.
