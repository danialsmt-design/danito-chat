...
**MAGAZINE-SLIP LOT START (Danial, 2026-09-07 — decided, NOT built; work stopped at Danial's "stop"):** from the next new lot on the second-side line, the operator scans the MCS slip QR (`PO: … | QTY: … | MAG: n/N | DATE: …`, no model/side in it) to open the lot and change model — **the QR replaces the supervisor badge for the lot start** (rule change to [[pvs-lot-supervisor-only]]); **side comes from the machine program** (C3P "L311 - B SIDE"); model/target from the DB by PO; then every magazine tag is scanned until lot end (pcs = QTY÷N, dedupe by MAG#, wrong-PO slip refused). Existing pieces: `/api/boardinput/scan` + `MagazineSlipParser` + `BoardInputState` (count-only). Build target: L1 canary. Open: mid-lot foreign-PO slip = block or warn; first-side line keeps pack-UID scan or gets a slip too.
**First side too (Danial, 2026-09-07):** the bare-PCB pack comes with a Part Number + UID label (e.g. `711009-I547%`; qty = StockOut by UID — the existing pack path) AND the same MCS slip/tag as the second side for lot scanning. So BOTH sides open the lot by slip-QR scan (no badge; side from the machine program); input differs only in what is scanned: first side = pack UID (part+qty from StockOut), second side = magazine tag (QTY÷N). Only the "QR opens the lot" piece was new — **BUILT + LIVE L1 2026-09-07 (`90f9b6f`)**: `StartLotFromSlipAsync` (guards: no model yet / unknown PO / wrong model / running lot short of target → refuse); main scanner routes a slip QR to `/api/boardinput/scan`.
