...
**BOM per-board = ProductBOM Quantity ONCE per side + side-order lift (2026-08-21, Danial):** `ProductBOM` stores the per-board mount `Quantity` per (part, side); it is replicated across multiple **Line** rows (and sometimes 2 feeders for the same side on different lines). RULE (Danial): "there is never 2 same part on same line" + "mount per board is in ProductBOM, don't confuse with line". So per-board qty per side = the `Quantity` taken ONCE per side (MAX across the line/feeder rows), NOT summed. `bomside` CTE now uses `MAX(CASE WHEN sd='A Side'…)` not SUM → e.g. L311 VE4-2850-104 A-side [F]126 & [F]110 both =10 → A=10 (was wrongly 20), total 30 (Canon says 30). L309 = A5+B23=28. SIDE ORDER: L311 runs B-side first, ALL other models A-side first; the first side's completion is LIFTED by the second side's production (second side can't run before first, so it implies the first is done) — fixes phantom first-side demand when the first side is under-recorded. Both in `/api/completability` baseCte, deployed.
**⚠️ ProductBOM has real errors vs Canon's BOM (the authority):** Aug-7 `MCS/bom-verify/*.csv` reconciled Canon BOM (`CanonQty`) vs DB for 6 Aug models — 17 qty MISMATCHes (e.g. L254 VW5-6454-104 Canon 31 vs DB 1 = under-count danger; WA6-3110-000 Canon 1 vs DB 31), 10 MISSING_IN_DB, 9 MISSING_IN_CANON; L261 Canon BOM UNREADABLE (37 unverified); **L309 has NO Canon BOM on file**. Canon's master BOM is NOT stored anywhere (only used to build the verify sheets) — get it from Danial/Ashish for a full re-check. Correction list for Ashish: `MCS/BOM-corrections-for-Ashish.csv` (75 rows). Full DB mount-point dump: `MCS/mount-points-by-model.csv`. Fix target = `ProductBOM` table in ReelPart-New on .134 (system of record). Note the completability carries these DB BOM errors until Ashish corrects ProductBOM.
