...
- [MCS DB sync status](mcs-db-sync-status.md) — go.html "DB sync" block + /api/sync from msdb.restorehistory; 2026-09-11: mirror stale since 09-09 19:50, Parts Control PC .134 gone from the LAN.
...
- [MCS Canon invoice capture](mcs-canon-invoice-capture.md) — capture Canon part-invoice scanned PDFs into DB `CanonPartInvoices` (table live on staging); pypdfium2 render + vision extract proven; user-keyed accounting month.
...
- [PVS: no SMB, use remoting](pvs-no-smb-use-remoting.md) — never drop files on line PCs via \ip\C$ shares; WinRM/PowerShell remoting over Tailscale/LAN only (deploy-pvs.ps1 still uses SMB internally — flag).
- [PVS deploy when line idle](pvs-deploy-when-line-idle.md) — idle = stop/state down + >=4 min since lastBoardAt (boardsPerHour NEVER decays after a stop, so rate=0 never fires); deploy-when-idle.ps1 watcher still has the old signal + line IPs.
...
- [PVS reel UID = whole number](pvs-reel-uid-whole-number.md) — UID is the COMPLETE number, never truncated first-digits; `0100-` is truncated `880100-`; the two formats made `0100-E384%` (5,509) look like stock when the loaded reel was `880100-G429%` — a phantom, operator was right, PVS didn't write it.
- [PVS spare vs loaded](pvs-spare-vs-loaded.md) — canonical: a reel on a feeder is PRODUCTION, never a spare; spare/forecast count = StockOut reels NOT on a feeder; inflated counts = stale rows to zero, not loaded reels.
- [PVS parts-out expected watchdog](pvs-partsout-expected-watchdog.md) — a running line MUST produce parts-outs; estimate cadence from per-board usage × boards vs UID remaining; prolonged silence = serial comm/parse break, an alarm not calm.
- [PVS port-cell alignment](pvs-port-cell-alignment.md) — machine# must equal the program CELL# it reports; mismatch = cross-wired COM ports; L4 was cross-wired (fixed 2026-08-18); wake-ups now include this check.
- [PVS downtime capture](pvs-downtime-capture.md) — auto per-cell parts-exhaust recovery + operator reason pop-up (machine/waiting-part/no-air/rest/scheduled) by line; BUILT 2026-08-18, tests pass, deploy 2026-08-19; break-windows + "waiting boards" reason still to add.
- [PVS downtime capture](pvs-downtime-capture.md) — auto per-cell parts-exhaust recovery + operator reason pop-up (machine/waiting-part/no-air/rest/scheduled) by line; BUILT 2026-08-18 not deployed; shift/break/coverage model (staggered lunch, line runs covered, night shift on-demand); breaks are a hint not auto-exclude.
- [PVS line health](pvs-line-health.md) — one /api/health verdict from 6 signals (serial/producing/program/check/DB heartbeat/downtime) + a health pill on the operator screen; BUILT 2026-08-18 not deployed.
- [PVS board flow (two sides)](pvs-board-flow.md) — double-sided: store→bare-board pack(part#+UID, qty=StockOuts)→first-side line→magazine(slip PO+MAG#, DB app→MCS)→second-side line scan→FCT; which side is first is production-decided & sticky; input count = FIRST machine (L3=M2); route label 5-A→1-B→FCT.
- [PVS system brain](pvs-system-brain.md) — the CONNECTED PVS model + symptom→cause diagnostic map (vault: DANOTTO/PVS-System-Brain.md); reason from it before acting, connect new facts back, don't make Danial re-teach.
- [PVS parts requests](pvs-parts-requests.md) — forecast → MCS request board on store Terminals 1+2 (45 min before run-out) → store keeper scans reels (=StockOut) + sends robot, or dismisses (90-min cool-down); live 2026-09-07.
- [Reel delivery robot (Reeman)](reel-delivery-robot.md) — MCS-app dispatcher (NAS) = single writer to the robot; PVS 🤖 Call-robot button only proxies; wait times live-editable; NAS dispatcher LIVE 2026-09-07 (robot 192.168.0.115, RSN v3.2 JSON status, points line-1..5/StandByPoint); PVS 🤖 button live on all 5 lines since 2026-09-07 18:12 (config hand-added per box); never run a second dispatcher; live SLAM map + lidar on robot.html.
- [Line-manning vision](line-manning-vision.md) — per-line camera presence + badge identity fused in MCS brain → manned/unmanned/by-whom + "running unattended" alarm; spec in MCS/line-manning-spec.md; face-ID prototype (InsightFace) at MCS/vision/faceid built 2026-09-06, deviates from the spec's no-face rule.
...
- [PVS feeder list is truth](pvs-feeder-list-is-truth.md) — placement count trusted 100%, never learned/adjusted; balance = loaded qty − list × line panels since load; early/late exhaust = reel/StockOut qty wrong, never the list.
...
- [Frequent contacts](frequent-contacts.md) — WhatsApp numbers: Raja Rao (prod mgr), Danish, Ashish, Kalsom, Danial.
