...
**RULE CHANGE 2026-09-07 (Danial): the MCS magazine-slip QR opens the lot — no badge.** From the next new lot, the operator scans the production slip QR on the verify page; PVS opens that PO (model+target from DeliveryDocuments, side from the machine program). Supervisor badge is still the only way to force-END a lot short of its target, and the slip is refused while such a lot runs; the dropdown remains as a fallback. Built + live L1 `90f9b6f`. See [[pvs-board-flow]].

**RULE 2026-09-25 (Danial): lot size is the truth; the operator closes the PVS-down gap at lot end.** "Total board count must equal lot size. Machine count is for reference and the parts-exhaust forecast. 10 boards before the lot end, check lot size vs actual count; if not matching, pop up a message to update the board count. Operators will check the counter near end lot and update the card." Built `1607026`: within 10 boards of lot size the operator screen pops the COUNT CHECK (Matches / Update with the M4 completed-PWB panels); ANY registered badge may answer (this is the one count write an operator may do), still through the capped adoption (never above target +10 %, never backwards — the L307 guard stays); force remains supervisor-only. Once per lot, persisted, audited `LotEndCheck`. See [[pvs-count-two-counters-and-cap]].
