...
**✅ AUGUST FULLY CAPTURED (2026-08-17)** into `MCS.dbo.CanonPartInvoices` — 3 DNs, 194 lines, **RM 1,141,280.15**: SC/DN/260729-010 (101 lines RM 718,065.97), 260806-002 (85 lines RM 417,499.79 — re-captured from PDF, Danial had me discard the earlier xlsx load), 260811-002 (8 lines RM 5,714.39). Extraction method that WORKED at scale: `pypdfium2` render each page → **dispatch a general-purpose subagent per invoice** to vision-read the PNGs and return strict JSON (skips the DELIVERY-ORDER pages that repeat items — the PDFs are "INVOICE + DO", DO would double-count). Cross-check: each subagent's summed amounts matched the invoice's printed TOTAL. Confirmed **YH4-32xx-008 parts = "One-chip microcomputer IC"** (programmed ICs @ RM 12.85), validating the earlier costing correction. Reports page `/api/reports/canoninvoices` shows it.
...
**Purpose (Danial 2026-09-09):** the Canon invoice exists to confirm that what was stocked in
matches what Canon invoiced, i.e. to catch a short shipment by Canon. It is NOT a planning or
shortage input — material planning uses the actual balance only (see [[mcs-line-schedule]]).
